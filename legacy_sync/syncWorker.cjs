...
async function processSyncQueue() {
  ...
  try {
    ...
    pendingItems.sort((a, b) => {
      ...
      const pA = parseSafe(a.payload);
      const pB = parseSafe(b.payload);
      const tA = pA.updated_at || pA.created_at || a.created_at || 0;
      const tB = pB.updated_at || pB.created_at || b.created_at || 0;
      return getTablePriority(a) - getTablePriority(b) || new Date(tA).getTime() - new Date(tB).getTime();
    });
