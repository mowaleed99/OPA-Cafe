...
export class SQLiteInventoryRepository extends BaseElectronRepository<InventoryItem> implements IInventoryRepository {
  ...
  async addInventoryItem(item: InventoryItem): Promise<void> {
    await this.dbDriver.insert('inventory_items', item as any);
  }

  async updateInventoryItem(id: string, updates: Partial<InventoryItem>): Promise<void> {
    await this.dbDriver.update('inventory_items', id, updates as any);
  }
  ...
  async recordStockMovement(movement: StockMovement): Promise<void> {
    await this.dbDriver.insert('stock_movements', movement as any);
  }
}
